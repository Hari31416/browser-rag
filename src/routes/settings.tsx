...
function SettingsComponent() {
  ...
  const handleEmbeddingModelChange = (modelId: string) => {
    if (modelId !== prefs.embeddingModelId) {
      setShowReindexWarning(true)
    } else {
      setShowReindexWarning(false)
    }
    handleSave({ embeddingModelId: modelId })
  }
  ...
  return (
    <div className="space-y-6 animate-fade-in max-w-4xl">
      ...
      {showReindexWarning && (
        <div className="bg-destructive/10 border border-destructive/20 text-destructive p-4 rounded-lg flex items-start gap-3 animate-pulse">
          <AlertTriangle className="h-4 w-4 shrink-0 mt-0.5" />
          <div className="space-y-1">
            <h4 className="font-semibold text-sm">Re-indexing Required</h4>
            <p className="text-xs leading-relaxed opacity-90">
              Changing the embedding model changes the vector dimensions. Existing indexed documents will not be searchable
              until they are deleted and uploaded again under this new model.
            </p>
          </div>
        </div>
      )}
      ...
      <div className="grid gap-6">
        ...
        <Card className="bg-card/25 border-border/40 backdrop-blur-md shadow-lg rounded-xl overflow-hidden">
          <CardHeader className="py-4 border-b border-border/30 bg-card/5">
            ...
            <CardDescription className="text-xs">Select local embedding model for index generation.</CardDescription>
          </CardHeader>
              ...
              {EMBEDDING_MODELS.map((model) => (
                <div
                  ...
                  className={`p-4 rounded-lg border bg-card/10 cursor-pointer transition-all duration-200 ${
                    prefs.embeddingModelId === model.id
                ...
                >
                  <div className="flex justify-between items-start">
                    <div className="space-y-0.5">
                      <h4 className="font-semibold text-xs text-foreground">{model.displayName}</h4>
                      <p className="text-[10px] text-muted-foreground">
                        {model.modelId} • {model.dimensions} dimensions
                      </p>
                    </div>
                    {prefs.embeddingModelId === model.id && (
